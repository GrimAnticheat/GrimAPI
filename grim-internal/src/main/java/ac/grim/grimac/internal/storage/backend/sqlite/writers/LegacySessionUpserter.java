...
final class LegacySessionUpserter implements SessionUpserter {
    ...
    LegacySessionUpserter(Connection c, TableNames t) throws SQLException {
        this.insertIgnore = c.prepareStatement(
                "INSERT OR IGNORE INTO " + t.sessions() + "(session_id, player_uuid, server_name, started_at, last_activity, "
    ...
    public void addBatch(UUID sessionId,
        ...
        insertIgnore.setString(6, grimVersion);
        insertIgnore.setString(7, clientBrand);
        insertIgnore.setInt(8, clientVersionPvn);
        insertIgnore.setString(9, serverVersionString);
        insertIgnore.setString(10, replayClipsJson);
        insertIgnore.addBatch();

        update.setString(1, serverName);
        update.setLong(2, lastActivityEpochMs);
        update.setString(3, grimVersion);
        update.setString(4, clientBrand);
        update.setInt(5, clientVersionPvn);
        update.setString(6, serverVersionString);
        update.setString(7, replayClipsJson);
        update.setBytes(8, sessionIdBytes);
        update.addBatch();
    }
